...
class WishlistMailer < ApplicationMailer
  def digest(user, wishlist_items_with_requesters)
    ...
    mail(to: @user.email_address, subject: "Wishlist Items — can you help?")
  end
  ...
  def wishlist_available(user, item, item_request)
    ...
    group = CommunityGroupService.preferred_group_for_item_and_user(item, user)
    @action_url = CommunityGroupService.public_url_for(
