module Group
  class GroupsController < AdminController
    ...
    def group_params
      params.require(:community_group).permit(:name, :group_description, :domain, :short_name, :public, :support_item_types, :logo, :remove_logo).to_h.with_indifferent_access
    rescue ActionController::ParameterMissing
      # Handle case where params might not be nested
      params.permit(:name, :group_description, :domain, :short_name, :public, :support_item_types, :logo, :remove_logo).to_h.with_indifferent_access
    end
  end
end
